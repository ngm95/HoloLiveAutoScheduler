...
public class VideoService {
	...
	public void checkUpcoming() {
		while (true) {
			...
			// DB에서 Upcoming, Live테이블에 없는 채널을 찾아서 리스트로 가져옴
			List<MemberDTO> memberList = videoDao.readNotInUpcomingOrLive();
			...
			for (MemberDTO member : memberList) {
				...
				try {
					...
					findLimit.add(Calendar.HOUR_OF_DAY, -2);		// 2시간 전~지금까지 예약된 동영상 하나를 받아옴
					...
					// 이틀 안에 예약된 동영상이 있으면 DB의 Upcoming 테이블에 저장
					if (!searchResultList.isEmpty()) {
						...
						videoDao.createUpcoming(new VideoDTO(member.getMemberName(), member.getChannelId(), searchResult.getId().getVideoId(), scheduledStartTime, searchResult.getSnippet().getThumbnails().getDefault().getUrl(), member.getApiKey()));
	...
	public void checkLive() {
		while (true) {
			...
			System.out.println("\t upcoming : " + upcomingList);
			
			// Upcoming 동영상에 대해 Videos:list로 확인
